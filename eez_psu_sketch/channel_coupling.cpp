...
static Type g_channelsCoupling = TYPE_NONE;

void setType(Type value) {
    if (g_channelsCoupling != value) {
        if (CH_NUM < 2) {
            return;
        }

        g_channelsCoupling = value;
        ...
        bp::switchChannelCoupling(g_channelsCoupling);
    }
}

Type getType() {
    return g_channelsCoupling;
}

float getUSet(const Channel &channel) { 
    if (g_channelsCoupling == TYPE_SERIES) {
...
float getISet(const Channel &channel) { 
    if (g_channelsCoupling == TYPE_PARALLEL) {
...
float getPowerLimit(const Channel& channel) {
    if (g_channelsCoupling != TYPE_NONE) {
